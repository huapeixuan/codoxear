...
- [x] 2.4 Add error mapping tests: broker `{error:"..."}` becomes handler 502 where Python raises `ValueError`; dead session cleanup path returns 404 where Python would remove stale sidecar.
...
- [x] 7.1 Implement `POST /api/sessions/{id}/send`: historical Pi resume handling if feasible, live broker `send`, Pi images forwarding, idle auto-stop heartbeat refresh, dead broker cleanup, and fallback enqueue when broker socket is unavailable but process is alive._
...
- [x] 9.8 Add contract tests for create codex/pi happy path with stub/fake broker command where possible, cwd required/creation errors, resume not found, delete unknown/success cleanup, and takeover not eligible._
