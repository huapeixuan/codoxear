...
- [x] 9.5 Implement `handlers::session_meta::ui_state` for `GET /api/sessions/{id}/ui_state` (+ v1): mirror Python behavior; Pi sessions call `broker_client::broker_ui_state`, non-Pi sessions return `502` because UI interactions are Pi-only.
