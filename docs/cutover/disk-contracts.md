...
- Current Phase 5 implementation has a gated Rust scan loop that writes Python-compatible pending ledger rows from existing Codex/Pi log normalizers, preserves same-slot replacement (`last_error:"replaced by newer message"`), keeps narration disabled semantics, and exposes listener heartbeat state in `/api/settings/voice.audio.*`.
- Rust creates or reuses `webpush_vapid_private.pem` as PKCS#8 PEM. Automated contract coverage verifies Rust-created PEM files load through Python `py_vapid.Vapid.from_file` and produce the same base64url uncompressed public key.
